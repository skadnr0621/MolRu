...
export default function ItemCard({ product }) {
   ...
   * @param {*} id saleId
   */
  const checkAddress = async (privKey) => {
    if (privKey === "") {
      alert("개인키를 입력해야 합니다.");
      return;
    }
    ...
    var resp = await getByTokenId(tokenId);

    const sale = resp.data.data;
    const sender = web3.eth.accounts.privateKeyToAccount(privKey);

    if (action === "cancel") {
      if (sale.seller_address != sender.address) {
        alert("판매자 계정의 개인키가 아닙니다.");
        return;
      }
      cancelItem(sale.sale_id);
    } else {
      ...
      const highestBidder = saleInfo[5];
      if (highestBidder != sender.address) {
        alert("구매자 계정의 개인키가 아닙니다.");
        return;
      }
      completeItem(sender.address);
    }
  };
  ...
  const cancelItem = async (id) => {
    ...
    try {
      ...
      var resp = await getByTokenId(tokenId);
      // alert("hi. this is cancel");

      const sale = resp.data.data;
      const saleContract = new web3.eth.Contract(COMMON_ABI.CONTRACT_ABI.SALE_ABI, sale.sale_contract_address);
      await saleContract.methods.cancelSales().send({ from: sale.seller_address });

      await saleContract.getPastEvents("SaleEnded", { fromBlock: "latest" }).then(async (events) => {
        console.log("saleContract SaleEnded result", events);

        await axios
          .delete(process.env.REACT_APP_BACKEND_HOST_URL + `/sales/${id}`)
          .then((result) => {
            console.log("sales delete result", result);
          })
          .catch((err) => console.log("Error while sales delete", err));
      });
    } catch (err) {
      console.log("Error while ItemCard checkAddress", err);
    } finally {
      setLoading(false);
      toggleApprove();
  ...
  const completeItem = async (pubKey) => {
    ...
    try {
      ...
      const sale = resp.data.data;
      const saleContract = new web3.eth.Contract(COMMON_ABI.CONTRACT_ABI.SALE_ABI, sale.sale_contract_address);
      // check whether the sale is ended or not
      if ((await saleContract.methods.getTimeLeft().call()) > 0) {
        alert("아직 판매가 종료되지 않았습니다.");
        return;
      }

      await saleContract.methods
        .confirmItem()
        .send({ from: pubKey, gas: 3000000 })
        .then((result) => console.log("saleContract confirmItem result", result))
        .catch((err) => console.log("saleContract confirmItem error", err));

      await saleContract.getPastEvents("SaleEnded", { fromBlock: "latest" }).then(async (events) => {
        console.log("saleContract SaleEnded events", events);

        const evt = events[0];
        const buyer = evt.returnValues.winner;
        await axios
          .patch(process.env.REACT_APP_BACKEND_HOST_URL + `/sales/${tokenId}/complete`, { buyer_address: buyer })
          .then((result) => console.log("sale complete result", result))
          .catch((err) => console.log("sale complete error", err));
      });
    } catch (err) {
      console.log("Error while ItemCard completeItem", err);
    } finally {
      setLoading(false);
      toggleApprove();
    }
              ...
              <Form autoComplete="off" noValidate onSubmit={handleSubmit} onReset={handleReset}>
                <Box sx={{ mt: 4, ml: 3, mr: 3 }}>
                  ...
                  <Stack direction="row" sx={{ mt: 3, mb: 3 }} justifyContent="center">
                    {loading === false ? (
                      <Button size="large" variant="contained" sx={{ width: "15%", fontSize: 16 }} onClick={toggleApprove}>
                        취소
                      </Button>
                    ) : null}
                    <LoadingButton size="large" variant="contained" type="submit" sx={{ ml: 3, width: "15%", fontSize: 16 }} loading={loading}>
