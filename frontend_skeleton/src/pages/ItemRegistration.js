...
import { Box, Button, Card, Container, Divider, Modal, Stack, TextField, Typography } from "@mui/material";
            ...
            <Form autoComplete="off" noValidate onSubmit={handleSubmit} onReset={handleReset}>
              <Stack spacing={3}>
                <Stack direction="row" alignItems="center">
                  <input
                    ...
                    style={{ display: "none" }}
                  />
                  <TextField sx={{ width: "60%" }} type="text" label="아이템 (업로드 확장자 형식: png, jpeg, jpg, gif)" value={itemName} disabled />
                  <Button sx={{ ml: 5, fontSize: 18 }} variant="contained" size="large" onClick={handleClick}>
                    찾기
                  </Button>
                </Stack>
                <TextField
              ...
              <Stack alignItems="center">
                <Button
                  ...
                  type="submit"
                  variant="contained"
                  onClick={Object.keys(touched).length && !Object.keys(errors).length && item.length !== 0 ? toggleApprove : null}
                >
                  등록
                </Button>
                      ...
                      <Form autoComplete="off" noValidate onSubmit={handleSubmit} onReset={handleReset}>
                        <Box sx={{ mt: 4, ml: 3, mr: 3 }}>
                          <Stack direction="row" sx={{ mt: 3 }}>
                            <TextField fullWidth type="text" label="개인키" onChange={handlePrivKey} value={privKey} />
                          </Stack>
                          <Divider sx={{ mt: 4 }} />
        ...
        <Container>
          <MotionContainer initial="initial" sx={{ mt: 5 }} open>
            <Box sx={{ maxWidth: 480, margin: "auto", textAlign: "center" }}>
              ...
              <Typography sx={{ color: "text.secondary" }}>판매 등록을 이어서 하시려면 다음 페이지로 이동해주세요.</Typography>

              <motion.div variants={varBounceIn}>
                <Box component="img" src="/static/illustrations/illustration_login.png" sx={{ height: 260, mx: "auto", my: { xs: 5, sm: 10 } }} />
              </motion.div>

              <Stack direction="row" justifyContent="space-between" sx={{ ml: 5, mr: 5 }}>
                <Button to="/" size="large" color="inherit" variant="contained" component={RouterLink} sx={{ fontSize: 16 }}>
                  그만하기
                </Button>
                <Button to={`/register/sale/${tokenId}`} size="large" variant="contained" component={RouterLink}>
                  다음 페이지로
                </Button>
              </Stack>
            </Box>
